...
function useSessionUser(): SessionUser | undefined {
  ...
  return (session.data?.data as { user?: SessionUser } | undefined)?.user
}
  ...
  return (
    <header className="border-border/70 bg-card relative overflow-hidden rounded-3xl border shadow-sm">
      ...
      <div className="relative flex items-center gap-4 px-6 py-6">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          {username ? (
            ...
            // Match the loaded layout's footprint: name (xl), handle (sm), email (xs).
            <div role="status" aria-busy className="space-y-1.5">
      ...
      {/* Username editing lives here; email + password are in the Account section.
          A Reveal so it slides open/closed (border included) instead of snapping. */}
      <Reveal open={editing}>
          ...
          <Button
            ...
            onClick={() =>
              void providerRedirect('google', {
                process: 'connect',
                callbackUrl: '/auth/callback?from=/settings/account',
              })
            }
          >
            Connect
